...
import subprocess, os, json, platform, custom_exceptions
...
def getOutput(commandline):
	...
	if system() == "Windows":
		try:
			return subprocess.check_output(commandline, startupinfo=status_info)
		except OSError:
			status_info = None
...
def probe(source):
	try:
		commandline = [
			...
			'-show_format',
			'-show_streams',
			'-show_chapters', os.path.realpath(source)
		]
		return json.loads(str(getOutput(commandline), 'utf-8'))
	except UnicodeEncodeError:
		raise custom_exceptions.invalidFilename(source)
...
def getJPEG_Image(source:str, size=None, quantizer=3, force=False):
	...
	if size is not None:
		...
		if force:
			...
			size=size.split('x')
			commandline += [
				('scale=w=\'min('+size[0]+', iw)\':h=\'min('+size[1]+', ih)\''+
				':force_original_aspect_ratio=decrease')
			]
